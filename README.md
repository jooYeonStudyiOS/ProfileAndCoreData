...
두 개 모두 데이터를 저장하고 관리하는 방법이지만 아래와 같은 차이점이 있다.  
...
## MVC를 목표로 구현했던 숙련 과제와 MVVM을 구현했던 ProfileViewController를 비교해보세요.
...
메인 화면에서 `go profile` 버튼틀 누르면 `UI&MVVM` 과제 메인 화면으로, `go core data` 버튼을 누르면 `데이터 일관성` 과제 메인 화면으로 이동하며, 각각의 화면에는 스토리보드를 새롭게 추가했습니다. 

### UI&MVVM 과제 화면
`UI 화면` 과 `MVVM 화면` 으로 나뉘며, 구조는 아래와 같습니다.

#### UI 페이지 : Main이 되는 `ProfileDesignViewController`가 있고, 필요에 따라 SubView를 추가하는 형식으로 진행. 오토레이아웃은 snpkit을 사용
...
### 데이터 일관성 과제 화면
`CoreDataManager`에서 CoreData의 CRUD를 모두 관리하게 했고, 싱글톤으로 설정했습니다.
